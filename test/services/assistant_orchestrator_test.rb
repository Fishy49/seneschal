...
class AssistantOrchestratorTest < ActiveSupport::TestCase
  ...
  test "run returns output and events on success" do
    fake_output = [
      { "type" => "assistant", "session_id" => "sess_abc", "message" => { "content" => [{ "type" => "text", "text" => "Hello from assistant" }] } }.to_json,
  ...
  test "run handles popen3 errors" do
    Open3.stub(:popen3, ->(*args, **kwargs, &block) { raise Errno::ENOENT, "claude not found" }) do
      orchestrator = AssistantOrchestrator.new(@conversation)
      result = orchestrator.run("test")
  ...
  def stub_popen3(stdout:, exit_code:, &block)
    stdin_mock = StringIO.new
    stdin_mock.define_singleton_method(:close) {}
    ...
    Open3.stub(:popen3, ->(*args, **kwargs, &blk) {
